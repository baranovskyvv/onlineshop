...
public interface RodCartRepo extends JpaRepository<RodCart,Long> {
    @Query("select NEW com.example.fishingstore.domain.LittleCart(o.id,p.name,o.price,o.amount,o.type) from Rod p, RodCart o " +
            "where   o.id_ord=:id_ord and p.id=o.id_prod")
    List<LittleCart> findAllProductInOrder(@Param("id_ord")Long id_ord);

    @Query("select SUM(O.price*O.amount) as price from RodCart O where O.id_ord= :o")
